...
function _serialize_uint8array(arr) {
    arr = Array.from(arr);
...
function isRegexExecMatchArray(arg) {
    return Array.isArray(arg)
        ...
        && Object.prototype.hasOwnProperty.call(arg, "groups")
...
function serializeRegexExecMatchAsString(arg) {
    ...
    const repr = "<re.Match object; span=(" + start + ", " + end + "), match=" + pythonReprString(matchText) + ">";
    return ["string", codePointLength(repr), repr];
}
...
function serializeArray(arg) {
    ...
    const serializedValsStr = JSON.stringify(normalizedVals);
...
function serializeMap(arg) {
    ...
    for (const [key, value] of arg.entries()) {
        ...
        if (key.hasOwnProperty("_class_name") && key._class_name === "memoize") {
            return serializeString("memoize");
        }
...
function serialize(arg) {
    ...
    if (Array.isArray(arg)) {
        if (isRegexExecMatchArray(arg))
            return serializeRegexExecMatchAsString(arg);
        return serializeArray(arg);
    }
    if (Object.prototype.toString.call(arg) === "[object Uint8Array]")
        return _serialize_uint8array(arg);
...
function myexactlog(...args) {
    ...
    fsystem.writeSync(1, JSON.stringify(info_list) + "\n");
    _trace_idx += 1
}
...
// dual of Python's hasattr built-in function
