...
def _serialize_byte_array(arg):
    arr = list(arg)
    return serialize_list(arr)
...
def serialize_dict(arg: dict):
    ...
    for key in keys:
        ...
        if isinstance(key, (int, float)):
            ...
            del argcp[key]
            continue
        if hasattr(key, '_class_name') and key._class_name == 'memoize':
            return serialize_str('memoize')
...
def serialize_regex_match_as_string(arg):
    match_as_str = str(arg)
    return ["string", len(match_as_str), match_as_str]
...
def serialize(arg):
    ...
    if isinstance(arg, bytes):
        return _serialize_byte_array(arg)
    ...
    if isinstance(arg, re.Match):
        return serialize_regex_match_as_string(arg)
    str_result = str(arg)
