...
function isRegexExecMatchArray(arg) {
    return Array.isArray(arg)
        && Object.prototype.hasOwnProperty.call(arg, "index")
        && Object.prototype.hasOwnProperty.call(arg, "input")
        && typeof arg.index === "number"
        && typeof arg.input === "string"
        && typeof arg[0] === "string";
}

function serializeRegexExecMatchAsUnknown(arg) {
    const matchText = arg[0];
    // Translation candidates can patch `index` to the absolute position
    // while leaving `input` as a sliced string. Use patched absolute index.
    const start = arg.index;
    const end = start + codePointLength(matchText);
    const repr = "<re.Match object; span=(" + start + ", " + end + "), match="
        + pythonReprString(matchText) + ">";
    return ["unknown", codePointLength(repr), repr];
}
...
function serializeDefinedInMain(arg) {
    // Root cause:
    // for instances, reading `arg._class_name` can be wrong because user code may
    // mutate that field (e.g. "HTMLParser;" + class_var._class_name), which
    // changes trc identity even when the runtime class is still ParserBase.
    // Fix rationale:
    // match Python serializer semantics by preferring runtime class identity
    // (`arg.__class__.__name__` in Python ~= `arg.constructor.name` in JS) and
    // only use `_class_name` as a fallback when constructor name is unavailable.
    const ctorName = (arg && arg.constructor) ? arg.constructor.name : undefined;
    if (typeof ctorName === "string" && ctorName !== "" && ctorName !== "Object") {
        return ["defined_in_main", ctorName];
    }
    if (Object.prototype.hasOwnProperty.call(arg, "_class_name") && typeof arg._class_name === "string") {
        return ["defined_in_main", arg._class_name];
    }
    return ["defined_in_main", "Object"];
}
...
function serialize(arg) {
    ...
    if (Array.isArray(arg)) {
        if (isRegexExecMatchArray(arg))
            return serializeRegexExecMatchAsUnknown(arg);
