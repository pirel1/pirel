...
function formatScientific6e(x) {
    ...
    let mantissa = Math.round(x * 1e6) / 1e6;
    let mantissaStr = mantissa.toFixed(6);
    // Remove trailing zeros and decimal if needed
    mantissaStr = mantissaStr.replace(/(\.\d*?[1-9])0+$/, "$1").replace(/\.0+$/, "");
    return sign + mantissaStr + "e" + (exp >= 0 ? "+" : "") + exp;
}
...
function serializeNum(arg) {
    ...
    if (arg >= Number.MIN_SAFE_INTEGER && arg <= Number.MAX_SAFE_INTEGER) {
        ...
        const srlzd = formatScientific6e(arg);
