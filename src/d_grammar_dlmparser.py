...
_SLOT_TOKEN_RE = re.compile(r'^"[*.](\d+)"$')


def _parse_slot_idx_from_placeholder_token(token: str) -> int:
  assert isinstance(token, str)
  m = _SLOT_TOKEN_RE.match(token)
...
class Expansion:
  ...
  def __init__(
    ...
    if d_consts.DEBUG_VERBOSE > -10: print("# Expansion create:", expand)
  ...
  def _fill(
    ...
    def _replace_and_visitor_inner_fun(node):
      ...
      if node.startswith('"_val'):
        ...
        idx_in_name = _parse_slot_idx_from_placeholder_token(node)
    ...
    fill_count = d_utils.traverse_nested_list_replace(
    ...
    if d_consts.DEBUG_VERBOSE > -10: print("# _fill copy-and-fill-expand fill_count:", fill_count, exp_fragment)
...
class DelimitedParser():
  ...
  def __init__(
    ...
    # Root cause:
    # Some grammar/choice paths do not converge quickly and can spend a long
    # time before hitting the parser safety assert.
    # Patch reason:
    # Make loop limit tunable from env so we can fail fast on bad paths
    # without code changes per run.
    _default_loop_limit = 24000
    try:
      self._LOOP_LIMIT = int(
        os.environ.get('PIREL_DLM_LOOP_LIMIT', str(_default_loop_limit))
      )
    except ValueError:
      self._LOOP_LIMIT = _default_loop_limit
    if self._LOOP_LIMIT < 1000:
      self._LOOP_LIMIT = 1000
    self._loop_idx = 0
    ...
    self._VERBOSE = d_consts.DEBUG_VERBOSE
  ...
  def _check_should_skip_symbol(self, symbol_name):
    return d_grammar.grm_is_skipped_NT(self.grammar, symbol_name)
  ...
  def _pretty_grammar_cont(self, grc):
    ...
    def _get_pretty_grammar(grc):
      return f"(GRC ntdepth({grc[0]}) pid({grc[1]}) prod({id(grc[2]) if grc[2] is not None else ''} {d_grammar.grm_prod_pretty(grc[2])}) c_idx({grc[3]}) s_idx({grc[4]})) => " + self._pretty_grammar_cont(grc[5])
  ...
  def _is_possible_prod_for_ex_contin(self, ex_contin, prod):
    ...
    if self._VERBOSE > -2: print("    --- PRETTY PROD:", d_grammar.grm_prod_pretty(prod))

    if d_grammar.SYM_AHEADSET_KEY not in prod:
      return True

    ahead_set = prod[d_grammar.SYM_AHEADSET_KEY]
  ...
  def add_expansion_parse_until_stuck(self, expansion: Expansion):
    ...
    while True:
      ...
      if self._is_grm_back_tracking:
        ...
        if self._VERBOSE > 0: print(f"BACKTRACKING CHECKING ------(at grm_nt_depth {bk_grm_nt_depth})-------", d_grammar.grm_prod_pretty(bking_prod), bk_grm_contin[3], bk_grm_contin[4])
      ...
      # prod_type case 7
        ...
        def _handle_matching_symbol_or_alias_inner_fun(expan_fragment_elem, symbol_name, alias_prod=None):
          ...
          if elem_nt_name == t_symbol_name:
            ...
            symbol_or_alias_prod = d_grammar.grm_get_prod(self.grammar, symbol_name) if alias_prod is None else alias_prod
            self._add_NT_tail(
              ex_nt_depth,  # ex_nt_depth
              ex_id,  # ex_id
        ...
        if prod_type == "SYMBOL":
          ...
          elif d_grammar.grm_is_external_NT(self.grammar, symbol_name):
