...
def write_yaml(
  ...
  write_file(fpath, yaml_str, include_timestamp=include_timestamp, is_inline=is_inline)
...
def write_file(
  ...
  '''
  assert isinstance(fpath, (Path, str)), f'Expected Path or str, got {type(fpath)}'
...
def _get_mailgun_credentials() -> Tuple[str, str, str]:
  ...
  try:
    ...
    mailgun_to_address = env_dict['MAILGUN_TO_ADDRESS']
    return mailgun_api_key, mailgun_domain, mailgun_to_address
...
def send_text_email(subject: str, message: str) -> bool:
  ...
  try:
    mailgun_api_key, mailgun_domain, mailgun_to_address = _get_mailgun_credentials()
  except KeyError:
